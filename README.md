...
- Eksport zawiera uczestników, ustawienia, planowany harmonogram, historyczne dyżury, ręczne dyżury, dni wolne oraz nieobecności uczestników.
- Import przywraca uczestników, ustawienia, planowany harmonogram, historyczne dyżury, ręczne dyżury i dni wolne.
- Import całkowicie zastępuje bieżący stan zapisany w przeglądarce.
- Pliki eksportowane w poprzednich wersjach aplikacji (bez pola `manualShifts`, `offDays` lub `participantAbsences`) są nadal obsługiwane — brakujące pola są uzupełniane pustą tablicą.
