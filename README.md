...
- Umożliwia ręczne oznaczenie wybranych dat jako **dni wolnych** (bez dyżuru). Dni wolne są pomijane przez generator — żaden dyżur nie jest tworzony na tak oznaczonej dacie — i nie wpływają na statystyki sprawiedliwości obciążeń uczestników. Dni wolne z datą wcześniejszą niż dzisiaj są wizualnie przygaszane w tabeli harmonogramu, analogicznie do dyżurów z przeszłą datą.
