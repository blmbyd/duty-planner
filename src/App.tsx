...
function App() {
  ...
  const handleAddSamples = async () => {
    ...
    if (result.length >= settings.peoplePerShift) {
      try {
        await scheduleState.generate(result, settings, historyState.historicalShifts, manualState.manualShifts, 'ignore-existing-positions', offDaysState.offDays, absencesState.absences)
  ...
  const handleAddOffDay = (offDay: OffDay): 'ok' | 'conflict' => {
    const occupiedDates = new Set([
      ...scheduleState.schedule.map((s) => s.date),
      ...manualState.manualShifts.map((s) => s.date),
    ])
